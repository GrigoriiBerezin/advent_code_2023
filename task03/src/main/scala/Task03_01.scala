...
object Task03_01 extends ZIOAppDefault with ReadFileSuite {
  ...
  private def parsePointsFromLine(line: String, yIndex: Int = 0): Seq[(Int, Int)] =
    line.zipWithIndex.foldLeft(Seq.empty[(Int, Int)]) {
      case (acc, (element, xIndex)) if !element.isDigit && element != '.' => (xIndex, yIndex) +: acc
      case (acc, _) => acc
    }

  private def getResult(numbers: Seq[NumberWithPosition], points: Seq[(Int, Int)]): UIO[Long] =
    numbers.filter(number => points.exists(number.isNearBy)).map(_.number).sum.succeed
}
